...
module fcc_executer #(
    ...
    (* mark_debug = "true" *) output reg                     o_cmd_ready,
    (* mark_debug = "true" *) input                          i_cmd_valid,
    input  [15 : 0]                i_cmd,
...
//    input       [15 : 0]           i_wid,
    ...
    (* mark_debug = "true" *) output  reg                    io_busy,
    (* mark_debug = "true" *) output  reg                    o_ce_n,
    (* mark_debug = "true" *) output  reg                    o_wp_n,
    (* mark_debug = "true" *) input                          i_rb_n,
    (* mark_debug = "true" *) output  reg                    o_we_n,
    (* mark_debug = "true" *) output  reg                    o_cle,
    (* mark_debug = "true" *) output  reg                    o_ale,
    (* mark_debug = "true" *) output  reg [ 3 : 0]           o_re,
    (* mark_debug = "true" *) output  reg                    o_dqs_tri_en, // 1 - output, 0 - input
    (* mark_debug = "true" *) input       [ 3 : 0]           i_dqs,
    (* mark_debug = "true" *) output  reg [ 3 : 0]           o_dqs,
    (* mark_debug = "true" *) output  reg                    o_dq_tri_en, // 1 - output, 0 - input
    (* mark_debug = "true" *) output  reg [31 : 0]           o_dq,
    (* mark_debug = "true" *) input       [31 : 0]           i_dq
);
...
// SLC DA-prefix control
reg        isfirst;     // 1 = NAND is in TLC mode, needs DA prefix before first R/W/E
reg        i1_isfirst;  // latched isfirst for phy_erase
reg        i2_isfirst;  // latched isfirst for phy_read
reg        i3_isfirst;  // latched isfirst for phy_prog
...
// isfirst: tracks whether NAND is in TLC mode (needs DA prefix for SLC).
// Arms at reset, re-arms when NAND Reset (0xFF) is issued (chip returns to TLC),
// clears after the first qualifying Read/Program/Erase command.
always@(posedge clk or posedge rst)
if(rst) begin
    isfirst <= 1'b1;
end else if(i_cmd_valid && o_cmd_ready && (i_cmd[7:0] == 8'hFF)) begin
    isfirst <= 1'b1;          // NAND Reset: chip returns to TLC, re-arm
end else if(i_cmd_valid && o_cmd_ready && isfirst &&
            ((i_cmd[7:0]  == 8'h00) ||   // Read (cmd1 of 30h/35h)
             (i_cmd[15:8] == 8'hD0) ||   // Erase (D0h is cmd2)
             (i_cmd[15:8] == 8'h10))) begin  // Program (10h is cmd2)
    isfirst <= 1'b0;
end
...
end else if(rd_data_merge_valid) begin // Read Page (0x30) and other read cmds
    o_res_valid <= 1'h1;
    o_res_data  <= 64'h0;
    o_res_id    <= rd_data_merge_id;      
