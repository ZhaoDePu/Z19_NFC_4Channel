...
module fcc_phy#(
    ...
    input                       clk,    // 400M
    input                       clk_div,// 100M, from MMCM, phase aligned with clk
    input                       clk_reset,
//    input                       clk_locked,
    input                       usr_rst,
...
wire [127:0] o_dq_two;
reg  [ 15:0] r_dqs_two;
(* mark_debug = "true" *)reg  [127:0] r_dq_two;

(* mark_debug = "true" *) reg                                match;
(* mark_debug = "true" *) reg  [ 2:0]                        dqs_rs; // right shift
(* mark_debug = "true" *) reg  [ 5:0]                        dq_rs;
(* mark_debug = "true" *) reg  [ 1:0] o_dqs_2b;
(* mark_debug = "true" *) reg  [63:0] o_dq_64b;
...
wire             [15 : 0] sync_i_dq;
(* mark_debug = "true" *) reg              [15 : 0] sync_o_dq;
...
//
// ####################################################################################
phy_dwidth_convert phy_dwidth_convert(
    .clk_a          (usr_clk          ),         
    .rst_a          (usr_rst          ),         
    .clk_b          (clk_div      ),                         
    .i_ce_n_a       (i_ce_n           ),         
...
sync_cell #(.C_SYNC_STAGE(7), .C_DW(2), .pTCQ(0)) 
    sync_cell_tri_t(.src_data({sync_i_dq_tri_en, sync_i_dqs_tri_en}), .dest_clk(clk_div), .dest_data({tri_t_dq, tri_t_dqs}));
...
//IDELAYCTRL #(
//   .SIM_DEVICE("ULTRASCALE_PLUS")  // Must be set to "ULTRASCALE_PLUS" 
//)
//IDELAYCTRL_inst (
...
// RB_N & CE_N - single way, direct instantiation
IBUF Inst_RBIBUF(
    .I(I_NAND_RB_N),
    .O(sync_o_rb_n)
);
...
phy_out #(
    ...
    .clk_div_in             (clk_div            ),
    .reset                  (clk_reset              ),
    .data_from_fabric       ({8{sync_i_ce_n}}        ),
    .data_to_pins_p         (O_NAND_CE_N             ),
    .data_to_pins_n         (                        )
);
...
if (usr_rst) begin
    ...
    dqs_rs <= 3'd0;
    dq_rs  <= 6'd0;
end else if(o_dqs_tmp_valid & (~match)) begin
