...
// i_addr[47:0]
//     {row4, row3, row2, row1, colum2, colum1}
...
//    input       [31 : 0]           i_dq
    input                          isfirst  // 1 = send DA prefix (SLC mode entry)
...
localparam
    ...
    CMD0  = 11'b100_0000_0000;  // DA SLC-mode prefix command
    ...
    case(state)
        IDLE: begin
            if(i_cmd_valid) begin
                state <= (isfirst && (i_cmd[7:0] == 8'h80 || i_cmd[7:0] == 8'h85)) ? CMD0 : CMD1;
            end
        end
        CMD0: begin  // DA SLC-mode prefix
...
if(rst) begin
    o_res_valid <= 1'h0;
    o_res_id    <= 16'h0;
end else if(((state_r == BUSY) || (state_r == WPST)) && (state == IDLE)) begin
    o_res_valid <= 1'h1;
    o_res_id    <= i_cmd_id;
end else if((state == WAIT) && (i_rb_n == 1'h1) && (~i_keep_wait)) begin // RB# high = program done
    o_res_valid <= 1'h1;
    o_res_id    <= i_cmd_id;
end else begin
    o_res_valid <= 1'h0;
...
if(rst) begin
    o_dq_tri_en <= 1'h1;
end else if( (state == CMD0) || (state == CMD1) || (state == ADDR) || (state == CMD2)|| (state == WPRE) || (state == DATA) )begin
