...
localparam
    ...
    CMD0  = 10'b10_0000_0000;  // DA SLC-mode prefix command
    ...
    case(state)
        IDLE: begin
            if(i_cmd_valid) begin
                state <= (isfirst && (i_cmd[7:0] == 8'h60)) ? CMD0 : CMD1;
            end
        end
...
if(rst) begin
    ...
    o_res_valid <= 1'h1;
    o_res_id    <= i_cmd_id;
end else if((state == WAIT) && (i_rb_n == 1'h1) && (~i_keep_wait) && (i_cmd[7:0] == 8'h60)) begin // ERASE Block: RB# high = done
    o_res_valid <= 1'h1;
    o_res_id    <= i_cmd_id;
end else begin
    o_res_valid <= 1'h0;
