...
sync_cell #(.C_SYNC_STAGE(7), .C_DW(2), .pTCQ(0)) 
    sync_cell_tri_t(.src_data({sync_i_dq_tri_en, sync_i_dqs_tri_en}), .dest_clk(clk_div_int), .dest_data({tri_t_dq, tri_t_dqs}));
...
always @(posedge clk_div_int or posedge usr_rst)
if (usr_rst) begin
